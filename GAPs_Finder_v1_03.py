...
def yfinance_func(nome_ticker):
    ...
    if  not dati_storici.empty:
        ...
        st.write('Titolo inesistente o delistato')
        return dati_storici
...
col1,col2,col3 = st.columns([0.1,0.47,0.43])   
...
with col1:
    ...
    with st.form(key=f'GAPs_Finder'):
         nome_ticker = st.text_input('**GAPs Finder v1.01**',placeholder='inserisci Ticker')   
    ...
    if bottone_ricerca:
        ...
        if nome_ticker:
            ...
            st.warning('non hai inserito il ticker')
    ...
    if 'dati_storici' in st.session_state and st.session_state['dati_storici'] is not None:
        ...
        with stampa_col1.container():
            ...
            st.write(
                f"""
                #### {nome_ticker.upper()} 
                {st.session_state['fondamentali_finvitz']['sector']}   
                {st.session_state['fondamentali_finvitz']['industry']}
                             
                **market cap**: {st.session_state['fondamentali_finvitz']['market cap']}  
                **shares float**: {st.session_state['fondamentali_finvitz']['shares float']}  
                **insider Own**: {st.session_state['fondamentali_finvitz']['insider Own.']}  
                **inst. Own**: {st.session_state['fondamentali_finvitz']['inst. Own.']}  
                **Short Float**: {st.session_state['fondamentali_finvitz']['Short Float']}  
                """ )
               
               
            
            #st.write( st.session_state['fondamentali_finvitz'])
            #st.write(st.session_state['dati_split'])
            
            if not st.session_state['dati_split'].empty:
                st.write("")
                st.write("**Splits / Reverse Splits**")
...
#dati_storici = yfinance_func(nome_ticker)
with col2:
